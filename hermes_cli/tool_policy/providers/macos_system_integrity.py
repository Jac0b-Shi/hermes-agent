...
def _is_local_macos_terminal() -> bool:
    ...
    if sys.platform != "darwin":
        return False
    # Scoped accessor: a multiplexed gateway may serve an SSH/Docker profile while
    # the process env still says local. Refusal scopes must fail closed.
    from tools.terminal_scope import TerminalPolicyUnavailable, terminal_env

    try:
        terminal_kind = terminal_env("TERMINAL_ENV", "local")
        terminal_backend = terminal_env("HERMES_TERMINAL_BACKEND", "")
    except TerminalPolicyUnavailable:
        return True
    return terminal_kind == "local" and terminal_backend not in {
